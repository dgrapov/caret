#' @title cat2
#' @details hack to replace cat with message to allow verbose output to shiny apps
cat2<-function(...){
  message(paste(...))
}
...
train.default <- function(x, y,
  ...
  if(trControl$method != "none") {
    ...
    if(all(is.na(performance[, metric]))) {
      cat2(paste("Something is wrong; all the", metric, "metric values are missing:\n"))
    ...
    if(trControl$verboseIter && nrow(performance) > 1) {
      cat2("Selecting tuning parameters\n")
      flush.console()
    }
  ...
  if(trControl$verboseIter) {
    ...
    cat2("Fitting", bestText, "on full training set\n")
    flush.console()
  }
  ...
  if(trControl$trim && !is.null(models$trim)) {
    ...
    if(trControl$verboseIter) {
      ...
      cat2("Final model footprint reduced by", reduction, "or", p_reduction, "\n")
    }
  }
