...
adaptiveWorkflow <- function(x, y, wts, info, method, ppOpts, ctrl, lev, 
  ...
  for(iter in ctrl$adaptive$min:length(resampleIndex)) {
    ...
    if(class(filtered_mods)[1] == "try-error") {
      if(ctrl$verboseIter) { 
        cat2("x parameter filtering failed:")
        print(filtered_mods)
        cat2("\n")
      }
    ...
    if(ctrl$verboseIter) {
      ...
      if(length(excluded) > 0) {  
        cat2(paste("o", length(excluded), "eliminated;"))
      } else cat2("o no models eliminated;", 
                 nrow(current_mods), 
    ...
    if(ctrl$verboseIter && length(excluded) > 0) cat2(num_left, 
...
skunked <- function(scores, verbose = TRUE) {
  ...
  if(any(by_player$wins < 1)) {
    ...
    if(verbose) cat2("o", sum(by_player$wins < 1), 
...
filter_on_diff <- function(dat, metric, cutoff = 0.01, maximize = TRUE, verbose = FALSE) {
  ...
  if(length(deletecol) > 0) {
    ...
    if (verbose)  cat2(paste("o", length(deletecol), 
